...
#define TASK_EXECUTION_PRIORITY       5
...
static void release_dd_task( dd_task *ddtask, uint32_t release_time );
...
int main(void)
{
	...
	while (xTaskGetTickCount() < pdMS_TO_TICKS(TASK1_EXECUTION_TIME)){} // TODO: SOmething is going wrong here
...
static void DD_Task_Generator_1( void *pvParameters )
{
	...
	while(1)
	{
		...
		created_dd_task = create_dd_task(NULL, PERIODIC, TASK1_ID, (xTaskGetTickCount() + pdMS_TO_TICKS(TASK1_PERIOD)));
			...
			switch(received_request){
				case TASK_CREATED:
					...
					if (task_to_execute != NULL) {
					    ...
					    vPortFree(task_to_execute);  // Free memory if no longer needed
					}
